...
const WEATHER_COORDS = {
  'Neo Química Arena': { lat: -23.5489, lon: -46.4774 },
  'Morumbi': { lat: -23.6000, lon: -46.7046 },
  'Maracanã': { lat: -22.9121, lon: -43.2302 },
  'Mineirão': { lat: -19.9209, lon: -43.9719 },
  'Beira-Rio': { lat: -30.0650, lon: -51.2428 },
  'default': { lat: -23.5489, lon: -46.4774 },
};
...
async function fetchWeather(venue, matchDate) {
  try {
    const coords = WEATHER_COORDS[venue] || WEATHER_COORDS.default;
    const dateStr = matchDate.toISOString().split('T')[0];
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${coords.lat}&longitude=${coords.lon}&hourly=temperature_2m,precipitation_probability,weathercode,windspeed_10m&timezone=America/Sao_Paulo&start_date=${dateStr}&end_date=${dateStr}`;
    ...
    return {
      temp: temps[hour] ?? temps[Math.min(hour, temps.length - 1)] ?? null,
      rain: prec[hour] ?? prec[Math.min(hour, prec.length - 1)] ?? 0,
      code: codes[hour] ?? codes[Math.min(hour, codes.length - 1)] ?? 0,
      wind: wind[hour] ?? wind[Math.min(hour, wind.length - 1)] ?? 0,
      icon: WMO_CODES[codes[hour]] || '🌤️',
    };
  } catch (e) { return null; }
}
...
function aplicarFiltro() {
  ...
  st.filtered = lista.filter(m => m.date >= now || m.isLive).sort((a, b) => a.date - b.date);
  render();
}
...
async function buscarJogos() {
  ...
  try {
    ...
    st.matches = st.all.filter(m => m.date >= now || m.isLive).sort((a, b) => a.date - b.date);
    ...
    const proximo = st.matches.find(m => !m.isLive);
    if (proximo && proximo.venue && proximo.venue !== 'Local a definir') {
      st.weather = await fetchWeather(proximo.venue, proximo.date);
    } else { st.weather = null; }
  } catch (err) {
  ...
  if (liveMatch) {
    ...
    html += `<div class="card card-destaque">
      ...
      <div class="destaque-info">
        ...
        ${st.weather ? `<div class="di-item weather-chip">${st.weather.icon} ${st.weather.temp !== null ? Math.round(st.weather.temp) + '°C' : '--'} • Chuva: ${st.weather.rain}% • Vento: ${Math.round(st.weather.wind)}km/h</div>` : ''}
      </div>
...
function renderClassif() {
  ...
  // For knockout competitions (Copa do Brasil, Paulista), show round-by-round progress
  const isKnockout = st.filter === 'copabrasil' || st.filter === 'paulista';
  if (tabelas.length === 0 && !isKnockout) {
    c.innerHTML = '<div class="empty"><p>Classificacao indisponivel.</p></div>';
    return;
  }

  if (isKnockout) {
    const compName = st.filter === 'copabrasil' ? 'Copa do Brasil' : 'Paulista';
    const jogos = st.all.filter(m => m.championshipKey === st.filter && m.status !== 'scheduled' && m.status !== 'live').sort((a, b) => a.round - b.round);
    if (jogos.length === 0) {
      c.innerHTML = `<div class="empty"><p>Nenhum jogo realizado na ${compName}.</p></div>`;
      return;
    }
    const resumo = { v: 0, e: 0, d: 0, gp: 0, gc: 0 };
    jogos.forEach(m => {
      if (m.isHome) { resumo.gp += m.homeGoals || 0; resumo.gc += m.awayGoals || 0; }
      else { resumo.gp += m.awayGoals || 0; resumo.gc += m.homeGoals || 0; }
      if (m.resultado === 'V') resumo.v++;
      else if (m.resultado === 'E') resumo.e++;
      else if (m.resultado === 'D') resumo.d++;
    });
    const lastJogo = jogos[jogos.length - 1];
    const ultResultado = lastJogo.resultado === 'V' ? '🟢 Classificado' : lastJogo.resultado === 'E' ? '🟡 Aguardando' : '🔴 Eliminado';
    let html = `<div class="section-title">${compName}</div>
      <div class="resumo" style="margin-bottom:16px">
        <div class="r-item"><span class="r-num">${jogos.length}</span><span class="r-lab">Jogos</span></div>
        <div class="r-item r-v"><span class="r-num">${resumo.v}</span><span class="r-lab">V</span></div>
        <div class="r-item r-e"><span class="r-num">${resumo.e}</span><span class="r-lab">E</span></div>
        <div class="r-item r-d"><span class="r-num">${resumo.d}</span><span class="r-lab">D</span></div>
        <div class="r-item"><span class="r-num">${resumo.gp}</span><span class="r-lab">GP</span></div>
        <div class="r-item"><span class="r-num">${resumo.gc}</span><span class="r-lab">GC</span></div>
      </div>
      <div class="historico-item" style="margin-bottom:16px;border-left-color:var(--gray-500)">
        <span class="h-data">${fmtData(lastJogo.date)}</span>
        <span class="h-adv">${lastJogo.opponent}</span>
        <span class="h-placar">${lastJogo.isHome ? lastJogo.homeGoals : lastJogo.awayGoals} x ${lastJogo.isHome ? lastJogo.awayGoals : lastJogo.homeGoals}</span>
        <span style="font-size:0.75rem;color:var(--gray-300)">${ultResultado}</span>
      </div>`;
    c.innerHTML = html;
    return;
  }
...
function renderEscalacao() {
  const c = $('#hamContent');
  const filtered = m => st.filter === 'todas' || m.championshipKey === st.filter;
  const comEscalacao = st.all.filter(m => filtered(m) && m.status !== 'scheduled' && m.status !== 'live' && m.matchId && st.lineups[m.matchId]).sort((a, b) => b.date - a.date);
  const proximos = st.all.filter(m => filtered(m) && (m.status === 'scheduled' || m.status === 'live') && m.matchId).sort((a, b) => a.date - b.date);
...
function renderHistorico() {
  const c = $('#hamContent');
  const filtered = m => st.filter === 'todas' || m.championshipKey === st.filter;
  const jogos = st.all.filter(m => filtered(m) && m.status !== 'scheduled' && m.status !== 'live').sort((a, b) => b.date - a.date);
  if (jogos.length === 0) { c.innerHTML = '<div class="empty"><p>Nenhum jogo realizado.</p></div>'; return; }
  const label = st.filter === 'todas' ? 'Todos os jogos do Corinthians na temporada' : `Jogos do Corinthians - ${COMP_KEYS.find(k => k.key === st.filter)?.label || st.filter}`;
  let html = `<p class="st-legenda" style="font-size:0.85rem;color:var(--gray-400)">${label}:</p>`;
  jogos.forEach(m => {
    const cls = m.resultado === 'V' ? 'r-v' : m.resultado === 'E' ? 'r-e' : m.resultado === 'D' ? 'r-d' : '';
...
function renderAproveitamento() {
  ...
  const filtered = m => st.filter === 'todas' || m.championshipKey === st.filter;
  const comps = {};
  st.all.filter(m => filtered(m) && m.status !== 'scheduled' && m.status !== 'live').forEach(m => {
    if (!comps[m.league]) comps[m.league] = { j: 0, v: 0, e: 0, d: 0, gp: 0, gc: 0 };
    const r = comps[m.league]; r.j++;
    if (m.isHome) { r.gp += m.homeGoals || 0; r.gc += m.awayGoals || 0; }
    else { r.gp += m.awayGoals || 0; r.gc += m.homeGoals || 0; }
...
function renderSequencia() {
  const c = $('#hamContent');
  const filtered = m => st.filter === 'todas' || m.championshipKey === st.filter;
  const ultimos = st.all.filter(m => filtered(m) && m.status !== 'scheduled' && m.status !== 'live').sort((a, b) => b.date - a.date).slice(0, 15);
  ...
  const label = st.filter === 'todas' ? 'Ultimos 15 jogos do Timao' : `Ultimos jogos - ${COMP_KEYS.find(k => k.key === st.filter)?.label || st.filter}`;
  let html = `<p style="font-size:0.85rem;color:var(--gray-400);margin-bottom:4px">${label}:</p>${sparkHtml}
...
function renderCalendario() {
  ...
  const grupos = {};
  st.all.filter(m => filtered(m)).forEach(m => {
